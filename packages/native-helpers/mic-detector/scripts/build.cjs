...
function run(executable, args) {
  try {
    ...
    if (result.status !== 0) {
      console.warn(`[mic-detector] ${executable} exited with code ${result.status}. Skipping native helper.`);
    }
  } catch (err) {
    console.warn(`[mic-detector] Failed to execute ${executable}: ${err.message}. Skipping native helper.`);
  }
}
