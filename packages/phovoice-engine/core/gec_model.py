...
class PureBertWordPieceTokenizer:
    """Tokenizer WordPiece thuần Python không phụ thuộc transformers hay tokenizers package."""
    def __init__(self, vocab_file: str, lowercase: bool = False, unk_token: str = "[UNK]", max_input_chars_per_word: int = 100):
        self.vocab = {}
        self.ids_to_tokens = {}
...
class _BertTokenizerCompat:
    def __init__(self, vocab_file, do_lower_case=False, model_max_length=1024, **kw):
        self.tokenizer = PureBertWordPieceTokenizer(vocab_file, lowercase=do_lower_case)
    ...
    def tokenize(self, text):
        if isinstance(text, list):
            out = []
            for w in text:
                out.extend(self.tokenizer.tokenize_word(w))
            return out
        return self.tokenizer.tokenize_word(text)
    ...
    def encode(self, text, add_special_tokens=True, max_length=None, truncation=False, **kw):
        ...
        ids = []
        for w in words:
            ids.extend(self.tokenizer.encode_word(w))
    ...
    def convert_tokens_to_ids(self, tokens):
        ...
        return [self.vocab.get(t, self.tokenizer.unk_id) for t in tokens]
    ...
    def add_tokens(self, tokens):
        for t in tokens:
            if t not in self.vocab:
                ...
                self.vocab[t] = new_id
                self.tokenizer.ids_to_tokens[new_id] = t
    ...
    def __call__(self, texts, return_tensors=None, padding=False,
            ...
            if is_split_into_words:
                for wi, word in enumerate(text):
                    if word in self.vocab:
                        ...
                        sub_ids = self.tokenizer.encode_word(word)
                        ids.extend(sub_ids)
                        wids.extend([wi] * len(sub_ids))
            else:
                words = text.split() if isinstance(text, str) else text
        ...
        if padding:
            ml = max((len(x) for x in all_ids), default=0)
            padded_ids, attn = [], []
            for x in all_ids:
                padded_ids.append(x + [self._pad_id] * (ml - len(x)))
                attn.append([1] * len(x) + [0] * (ml - len(x)))
            all_ids = padded_ids
        else:
            attn = [[1] * len(x) for x in all_ids]

        tt_ids = [[0] * len(x) for x in all_ids]
        if return_tensors == "np":
            return _BatchEncoding(np.array(all_ids, dtype=np.int64),
                                  np.array(attn, dtype=np.int64),
                                  np.array(tt_ids, dtype=np.int64),
                                  all_word_ids)
        return _BatchEncoding(all_ids, attn, tt_ids, all_word_ids)

class AutoTokenizer:
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        vocab_file = os.path.join(pretrained_model_name_or_path, "vocab.txt") if os.path.isdir(pretrained_model_name_or_path) else pretrained_model_name_or_path
        return _BertTokenizerCompat(vocab_file, **kwargs)
