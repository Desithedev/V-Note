...
def get_verb_form_dicts():
    candidates = [
        os.path.join(VOCAB_DIR, "vocabulary", "verb-form-vocab.txt"),
        os.path.join(VOCAB_DIR, "verb-form-vocab.txt"),
        os.path.join(VOCAB_DIR, "models", "vibert-capu", "verb-form-vocab.txt"),
        os.path.join(os.environ.get("PHOVOICE_MODELS_DIR", ""), "vibert-capu", "verb-form-vocab.txt"),
    ]
    path_to_dict = next((p for p in candidates if os.path.exists(p)), None)
    if not path_to_dict:
        return {}, {}
    encode, decode = {}, {}
    with open(path_to_dict, encoding="utf-8") as f:
        for line in f:
            ...
            if not line_s or ":" not in line_s:
                continue
            words, tags = line_s.split(":", 1)
            if "_" not in words or "_" not in tags:
                continue
            word1, word2 = words.split("_", 1)
            parts = tags.split("_")
            if len(parts) < 2:
                continue
